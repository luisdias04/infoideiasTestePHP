...
class AnoParaSeculo
{
    ...
    function __construct ($anoParaSeculo) {
        $a = 
        new ValidarEntradaDeValores ($anoParaSeculo) ;
        $this->seculoValido =
        ($a->anoValidado == -1) ? false : true ;
        if (!$this->seculoValido || !is_numeric ($anoParaSeculo)) {
            $this->seculo =
            "Ano inválido ou não faz parte dos numeros naturais maior que zero!" ;            
        } else {
            $b=
            array_reduce ((array) new ConverterNumerosParaArray ($a->anoValidado), 'array_merge', array()) ;
            $c =
            new SomaPorCriterio ($b) ;
            $d =
            new SufixoSeculoParaSomar ($b) ;
            $this->ValidarEntradaDeValores =
            $a ;
            $this->ConverterNumerosParaArray =
            $b ;
            $this->SomaPorCriterio =
            $c ;
            $this->SufixoSeculoParaSomar =
            $d ;
            $this->getSeculo() ;
        }
    }
